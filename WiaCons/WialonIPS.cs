...
namespace WialonIPS
{
    ...
    public class MessagesCommunicator : IDisposable
    {
        ...
        public MessagesCommunicator(string host, UInt16 port)
        {
            this.Host = host;
            this.Port = port;
            this._client = new TCPClient(this.Host, this.Port);
        ...
        {
            // Костыли, убирающие визуальный интерфейс или мы убираем костыли визуального интерфейса
            //this._err_reporter.OnException(String.Format("TCPClient {0} exception", client), exc);
            Console.WriteLine("TCPClient "+ client + " exception || " + exc);
        }
    ...
    {
        Login, LoginAns, Data, DataAns, Ping, PingAns, ShortData, ShortDataAns,
